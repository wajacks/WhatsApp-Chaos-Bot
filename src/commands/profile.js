...
function getEconomyUserId(message) {
...
async function handleProfileCommand(message, client) {
    ...
    const senderId = getEconomyUserId(message);
...
━━━━━━━━━━━━━━━━━━`;
    ...
    if (
        ...
        topPlayers.forEach((p, index) => {
            lbMessage +=
                `${medals[index]} *${p.name}* — Lvl ${p.level || 1} (${p.xp || 0} XP)\n`;
        });
